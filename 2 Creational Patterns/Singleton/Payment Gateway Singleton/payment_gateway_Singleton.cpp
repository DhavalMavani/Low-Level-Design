...
private:
    PaymentGatewayManager() { cout << "Payment Gateway Manager initialized." << endl; }
...
public:
    static PaymentGatewayManager *getInstance(){
        if (instance == nullptr){
            mtx.lock();
            if (instance == nullptr) instance = new PaymentGatewayManager();
            mtx.unlock();
        }
        return instance;
    }

    void processPayment(double amount) { cout << "Processing payment of $" << amount << " through the payment gateway." << endl; }
};
...
int main(){
    ...
    if (paymentGateway == anotherPaymentGateway) cout << "Both instances are the same. Singleton pattern is working" << endl;
    else cout << "Singleton pattern is not working correctly" << endl;
